...
export class Game {
    ...
    for (const row of this.tiles) {
      for (const tile of row) {
        ...
        if (!tile.realm) {
          ...
          while (
            villages.length > 1 ||
            (villages.length > 0 && tile.realm.length < 2)
          ) {
    ...
    while (toSearch.length > 0) {
      ...
      for (let i = 0; i < 6; i++) {
        ...
        if (considered && search.player === considered.player) {
          ...
          if (
            searched.indexOf(considered) === -1 &&
            toSearch.indexOf(considered) === -1
          ) {
          ...
          while (tile.village.balance < 0) {
            ...
            for (const deathTile of realm) {
              if (deathTile.pawn && deathTile.pawn.upkeep > 0) {
                ...
                this.message(
                  deathTile.player,
                  'Your unit died from unmet upkeep.'
                );
              }
            }
          }
   ...
   */
  calculateRealmStats(set: Tile[]): Realm {
    let village = null;
    let upkeep = 0;
    for (const tile of set) {
      if (tile.pawn) {
    ...
    return { upkeep, income: set.length, village, tiles: set };
  }
